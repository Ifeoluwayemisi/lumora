...
function RegisterContent() {
  ...
  useEffect(() => {
    if (searchParams) {
      ...
      if (queryError) {
        console.log("[REGISTER PAGE] Error from OAuth:", queryError);
        const errorMessage =
          queryMessage ||
          (queryError === "account_exists"
            ? "An account already exists. Please sign in with your email instead."
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      const response = await api.get("/auth/google/url?intent=signup");
      const { authUrl } = response.data;
      window.location.href = authUrl;
    } catch (err) {
      console.error("[GOOGLE_SIGNUP] Error:", err);
      let errorMsg =
      ...
      if (err.response?.status === 503) {
        errorMsg =
          "Google OAuth not configured. Please create an account manually.";
      } else if (
        errorMsg.includes("not configured") ||
        errorMsg.includes("admin")
      ) {
        errorMsg = errorMsg + " Please create an account manually instead.";
      }
