...
export default function TeamPage() {
    ...
    const getManufacturerId = async () => {
      try {
        ...
        if (data.manufacturer?.id) {
          ...
          await fetchTeamData(data.manufacturer.id);
        }
      } catch (error) {
  ...
  const fetchTeamData = async (mfgId) => {
    try {
      ...
      const [membersRes, invitesRes] = await Promise.all([
        fetch(`/api/manufacturer/${mfgId}/team`),
        fetch(`/api/manufacturer/${mfgId}/team/invites`),
      ]);
  ...
  const handleSendInvite = async () => {
    ...
    try {
      const response = await fetch(
        `/api/manufacturer/${manufacturerId}/team/invite`,
        {
          method: "POST",
  ...
  const handleUpdateRole = async () => {
    ...
    try {
      const response = await fetch(
        `/api/manufacturer/${manufacturerId}/team/${selectedMember.id}/role`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const handleRemoveMember = async (memberId) => {
    ...
    try {
      const response = await fetch(
        `/api/manufacturer/${manufacturerId}/team/${memberId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleCancelInvite = async (inviteId) => {
    ...
    try {
      const response = await fetch(
        `/api/manufacturer/${manufacturerId}/team/invites/${inviteId}`,
        {
          method: "DELETE",
        },
      );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            ...
            <div className="p-6 space-y-4">
              ...
              <p className="text-sm text-gray-600 bg-gray-50 p-3 rounded-lg">
                This change will take effect immediately. The team member will
                see their updated permissions on next login.
              </p>
            </div>
