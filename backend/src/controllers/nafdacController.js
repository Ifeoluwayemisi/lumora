...
export async function getProductsList(req, res) {
    try {
        ...
        const enriched = products.map(code => ({
            ...
            scans: code.scans || 0,
            status: code.scans > 5 ? 'HIGH_RISK' : code.scans > 2 ? 'SUSPICIOUS' : 'SAFE'
        }));
...
export async function getReportsList(req, res) {
    try {
        ...
        const enriched = reports.map(report => ({
            ...
            priority: report.createdAt > new Date(Date.now() - 24 * 60 * 60 * 1000) ? 'CRITICAL' : 'HIGH',
...
export async function getManufacturersList(req, res) {
    try {
        ...
        const enriched = manufacturers.map(mfg => ({
            id: mfg.id,
            name: mfg.name,
            status: mfg.flagged ? 'SUSPENDED' : mfg.verified ? 'COMPLIANT' : 'WARNING',
